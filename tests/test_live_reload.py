...
class TestRenderTemplateLiveReload:
    ...
    def test_render_template_with_live_reload(self):
        ...
        response = render_template(
            "test.html",
            {"name": "World"},
            template_engine=Jinja2TemplateEngine(template_dir=self.temp_dir, live_reload=True)
        )
        
        # Check response
        assert response.status_code == 200
        assert "Hello World!" in response.content
        assert any(k == b'content-type' and b'text/html' in v for k, v in response.headers)
    ...
    def test_render_template_without_live_reload(self):
        ...
        response = render_template(
            "test.html",
            {"name": "World"},
            template_engine=Jinja2TemplateEngine(template_dir=self.temp_dir, live_reload=False)
        )
