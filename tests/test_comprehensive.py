...
from qakeapi.utils.validation import DataValidator, DictValidator, StringValidator, IntegerValidator
...
class TestComprehensiveAPI:
    ...
    async def test_validation_success(self, app):
        ...
        receive = AsyncMock()
        receive.side_effect = [
            {"type": "http.request", "body": b'{"name": "John", "age": 30}', "more_body": False}
        ]
    ...
    async def test_validation_error(self, app):
        ...
        receive = AsyncMock()
        receive.side_effect = [
            {"type": "http.request", "body": b'{"name": "", "age": -1}', "more_body": False}
        ]
