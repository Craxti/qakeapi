...
class TestWebSocketAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_connection_wrong_message_type(self, middleware, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_authenticate_connection_max_attempts(self, middleware, mock_websocket):
        """Test maximum authentication attempts."""
        # Set up max attempts
        middleware._auth_attempts[mock_websocket.connection_id] = middleware.config.max_auth_attempts
...
class TestWebSocketAuthHandler:
    ...
    @pytest.mark.asyncio
    async def test_require_auth_authenticated(self, handler, mock_websocket, middleware):
        """Test require_auth decorator with authenticated user."""
        # Add to authenticated connections
        auth_result = AuthResult(status=AuthStatus.AUTHENTICATED, user_id="user123")
        middleware._authenticated_connections[mock_websocket.connection_id] = auth_result
    ...
    @pytest.mark.asyncio
    async def test_optional_auth_authenticated(self, handler, mock_websocket, middleware):
