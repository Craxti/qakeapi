...
class TestResponse:
    ...
    async def test_response_stream(self):
        ...
        assert any(
            h == (b"transfer-encoding", b"chunked") for h in messages[0]["headers"]
        )
    ...
    async def test_response_stream_with_media_type(self):
        ...
        response = Response.stream(stream_data(), media_type="application/xml")
    ...
    async def test_response_stream_with_custom_headers(self):
        ...
        response = Response.stream(stream_data(), headers={"x-custom": "value"})
