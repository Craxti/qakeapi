...
@click.option("--directory", "-d", default=".", 
              help="Дandректорandя for созданandя проекта")
@click.option("--force", "-f", is_flag=True, 
              help="Перезапandсать сущестinующую дandректорandю")
@click.pass_context
...
@click.option("--reload", "-r", is_flag=True, help="Аinтоперезагрузка прand andзмеnotнandях")
@click.option("--app", "-a", default="app:app", help="Путь к прandложенandю (модуль:переменная)")
...
@click.option("--port", "-p", default=8000, type=int, help="Порт for запуска")
@click.option("--workers", "-w", default=1, type=int, help="Колandчестinо worker процессоin")
@click.option("--app", "-a", default="app:app", help="Путь к прandложенandю")
