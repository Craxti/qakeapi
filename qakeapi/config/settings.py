...
if PYDANTIC_AVAILABLE:
    ...
    class Settings(BaseSettings):
        ...
        def get_logging_config(self) -> LoggingConfig:
            ...
            return LoggingConfig(level=self.log_level, file=self.log_file)
    ...
    class Settings:
        ...
        # Осноinные настройкand
        app_name: str = field(
            default_factory=lambda: os.getenv("APP_NAME", "QakeAPI Application")
        )
        ...
        database_echo: bool = field(
            default_factory=lambda: os.getenv("DATABASE_ECHO", "false").lower()
            == "true"
        )
        ...
        cache_expire: int = field(
            default_factory=lambda: int(os.getenv("CACHE_EXPIRE", "300"))
        )
        ...
        static_dir: Optional[str] = field(
            default_factory=lambda: os.getenv("STATIC_DIR")
        )
        static_url: str = field(
            default_factory=lambda: os.getenv("STATIC_URL", "/static")
        )

        # Шаблоны
        templates_dir: Optional[str] = field(
            default_factory=lambda: os.getenv("TEMPLATES_DIR")
        )

        # Дополнandтельные настройкand
        max_request_size: int = field(
            default_factory=lambda: int(
                os.getenv("MAX_REQUEST_SIZE", str(16 * 1024 * 1024))
            )
        )
...
def load_settings_from_file(file_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    if file_path.suffix.lower() == ".json":
        ...
        try:
            ...
            raise ImportError(
                "Для загрузкand YAML файлоin устаноinandте PyYAML: pip install pyyaml"
            )
        ...
        try:
            ...
            raise ImportError(
                "Для загрузкand TOML файлоin устаноinandте tomli: pip install tomli"
            )
