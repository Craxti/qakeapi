...
class ValidationResult:
    ...
    def __init__(self, is_valid: bool, errors: Optional[List[str]] = None, data: Any = None):
...
class StringValidator(BaseValidator):
    ...
    def _validate_value(self, value: Any, field_name: str) -> ValidationResult:
        ...
        if self.min_length is not None and len(value) < self.min_length:
            errors.append(f"{field_name} must be at least {self.min_length} characters long")
        
        if self.max_length is not None and len(value) > self.max_length:
            errors.append(f"{field_name} must be at most {self.max_length} characters long")
...
if PYDANTIC_AVAILABLE:
    def validate_with_pydantic(model_class: Type[BaseModel], data: Dict[str, Any]) -> ValidationResult:
