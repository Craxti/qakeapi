...
class WebSocketAuthenticator(ABC):
    ...
    async def authenticate(
        self, websocket: WebSocketConnection, auth_data: Dict[str, Any]
    ) -> AuthResult:
...
class JWTAuthenticator(WebSocketAuthenticator):
    ...
    async def authenticate(
        ...
        try:
            ...
            if not token:
                return AuthResult(
                    status=AuthStatus.UNAUTHENTICATED, error_message="No token provided"
                )

            # Check if token is revoked
            if token in self._revoked_tokens:
                return AuthResult(
                    status=AuthStatus.INVALID, error_message="Token has been revoked"
                )
    ...
    async def authenticate_connection(
        ...
        try:
            ...
            message = await asyncio.wait_for(
                websocket.receive_json(), timeout=self.config.auth_timeout
            )
            ...
            return AuthResult(
                status=AuthStatus.INVALID, error_message="Authentication failed"
            )
...
class AuthenticatorFactory:
    ...
    def create_auth_middleware(
        authenticator: WebSocketAuthenticator, config: AuthConfig
