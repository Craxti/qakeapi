...
class InMemoryRateLimiter(RateLimiter):
    ...
    async def is_allowed(self, key: str) -> Tuple[bool, RateLimitInfo]:
        ...
        self.storage[key] = {
            ts: count
            for ts, count in self.storage[key].items()
        ...
        info = RateLimitInfo(
            remaining=max(
                0, self.requests_per_minute - current_requests - 1
...
class RateLimitMiddleware:
    ...
    def __init__(
        ...
        key_func=lambda request: request.client[
            0
        ],  # Используем IP адрес из кортежа client
    ):
