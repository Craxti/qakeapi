...
class InMemoryRateLimiter:
    ...
    def _get_lock(self) -> asyncio.Lock:
        """Получить lock, создав его при необходимости"""
    ...
    async def is_allowed(
        ...
        async with self._get_lock():
            ...
            reset_time = (
                int((window_start + rule.window_ms) / 1000)
                if requests_queue
                else int(now / 1000)
            )
...
class RedisRateLimiter:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379"):
        try:
            ...
            raise ImportError(
                "Для andспользоinанandя RedisRateLimiter устаноinandте redis: pip install redis"
            )
    ...
    def _find_rule(self, request: Request) -> RateLimitRule:
        ...
        for pattern, rule in self.rules.items():
            if "*" in pattern:
                ...
                if re.match(regex_pattern, path) or re.match(
                    regex_pattern, f"{method} {path}"
                ):
                    return rule
...
STRICT_RATE_LIMIT = RateLimitRule(
    requests=10, window=60, message="Too many requestоin. Попробуйте позже."
)

MODERATE_RATE_LIMIT = RateLimitRule(
    requests=100, window=60, message="Преinышен лandмandт requestоin in мandнуту."
)

LENIENT_RATE_LIMIT = RateLimitRule(
    requests=1000, window=60, message="Преinышен лandмandт requestоin."
)
