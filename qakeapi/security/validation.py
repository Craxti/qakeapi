...
def create_string_validator(
    ...
    def validator(value: str) -> str:
        ...
        if len(value) < min_length:
            raise ValueError(f"Строка должна contain мandнandмум {min_length} сandмinолоin")
        
        if len(value) > max_length:
            raise ValueError(f"Строка должна contain максandмум {max_length} сandмinолоin")
        ...
        if forbidden_chars and any(char in value for char in forbidden_chars):
            raise ValueError(f"Строка содержandт deniedные сandмinолы: {forbidden_chars}")
...
password_validator = create_string_validator(
    min_length=8,
    max_length=128
)
