...
try:
    ...
    class CryptContext:
        ...
        def verify(self, password, hash_val):
            return self.hash(password) == hash_val
...
class PasswordManager:
    ...
    def __init__(self, config: Optional[SecurityConfig] = None):
        ...
        if PASSLIB_AVAILABLE:
            try:
                ...
                warnings.warn(
                    "Bcrypt unavailable, using simple hash (unsafe for production!)"
                )
                self.pwd_context = None
                self._use_simple_hash = True
        else:
            warnings.warn(
                "Passlib unavailable, using simple hash (unsafe for production!)"
            )
            self.pwd_context = None
            self._use_simple_hash = True
    ...
    def validate_password_strength(self, password: str) -> None:
        ...
        if self.config.password_require_digits and not any(
            ...
            raise HTTPException(
                status.BAD_REQUEST, "Password must contain at least one digit"
            )
    ...
    def verify_token(self, token: str, token_type: str = "access") -> TokenData:
        ...
        try:
            ...
            if exp is None or datetime.utcnow() > datetime.fromtimestamp(exp):
                raise HTTPException(status.UNAUTHORIZED, "Токен andстек")
            ...
            raise HTTPException(
                status.UNAUTHORIZED, f"Не удалось inалandдandроinать токен: {str(e)}"
            )
