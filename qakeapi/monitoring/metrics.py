    ...
    def get_lifetime_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "top_paths": dict(sorted(self._paths.items(), key=lambda x: x[1], reverse=True)[:10]),
    ...
    async def metrics_endpoint():
        ...
        return {
            "current": {
                ...
                "top_paths": dict(sorted(current_metrics.paths.items(), key=lambda x: x[1], reverse=True)[:10]),
            },
...
def create_prometheus_endpoint(collector: MetricsCollector) -> Callable:
    ...
    async def prometheus_endpoint():
        ...
        lines.append(f"# HELP qakeapi_request_duration_seconds Request duration in seconds")
        lines.append(f"# TYPE qakeapi_request_duration_seconds histogram")
        lines.append(f"qakeapi_request_duration_seconds_sum {current_metrics.total_duration}")
        lines.append(f"qakeapi_request_duration_seconds_count {current_metrics.total_requests}")
        ...
        lines.append(f"qakeapi_requests_per_second {current_metrics.requests_per_second}")
        ...
        for status_code, count in current_metrics.status_codes.items():
            lines.append(f'qakeapi_responses_total{{status_code="{status_code}"}} {count}')
