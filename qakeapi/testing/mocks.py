    ...
    async def start(self, port: int = 8080) -> None:
        ...
        for service_name, service in self.services.items():
            self._server.router.add_route(
                "*", f"/{service_name}/{{path:.*}}", self._handle_request
            )
    ...
    async def _handle_request(self, request: "web.Request") -> "web.Response":
        ...
        if isinstance(response.body, dict):
            return web.json_response(
                response.body, status=response.status, headers=response.headers
            )
        else:
            return web.Response(
...
class MockHTTPClient:
    ...
    async def _request(
        ...
        service_path = (
            "/".join(parsed_url.split("/")[1:])
            if len(parsed_url.split("/")) > 1
            else ""
        )
...
def create_user_service() -> MockService:
    ...
    service.add_response(
        ...
        "/users/1",
        MockResponse(
            status=200, body={"id": 1, "name": "John Doe", "email": "john@example.com"}
        ),
    )
    ...
    service.add_response(
        ...
        "/users",
        MockResponse(
            status=201, body={"id": 3, "name": "New User", "email": "new@example.com"}
        ),
    )
...
def create_payment_service() -> MockService:
    ...
    service.add_response(
        ...
        "/payments",
        MockResponse(
            status=200, body={"transaction_id": "txn_123", "status": "success"}
        ),
    )
...
def create_email_service() -> MockService:
    ...
    service.add_response(
        "POST",
        "/email/send",
        MockResponse(status=200, body={"message_id": "msg_456", "status": "sent"}),
    )
