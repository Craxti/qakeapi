...
class LoadTester:
    ...
    async def run_load_test(self, config: LoadTestConfig) -> LoadTestResult:
        ...
        logger.info(
            f"Starting load test: {config.concurrent_users} users for {config.duration}s"
        )
        ...
        successful_requests = sum(
            1 for code in status_codes.values() if 200 <= code < 400
        )
        ...
        logger.info(
            f"Load test completed: {total_requests} requests, {requests_per_second:.2f} req/s"
        )
...
def create_simple_load_test(
    url: str, users: int = 10, duration: float = 60.0
) -> LoadTestConfig:
    """Create simple load test configuration."""
    return LoadTestConfig(
        url=url, method="GET", concurrent_users=users, duration=duration
    )
...
def create_stress_test(
    url: str, users: int = 50, duration: float = 300.0
) -> LoadTestConfig:
...
def create_spike_test(
    url: str, users: int = 100, duration: float = 60.0
) -> LoadTestConfig:
...
async def quick_load_test(
    url: str, users: int = 5, duration: float = 10.0
) -> LoadTestResult:
    """Quick load test for development."""
    config = LoadTestConfig(url=url, concurrent_users=users, duration=duration)
...
async def benchmark_load_test(
    url: str, users_range: List[int] = [1, 5, 10, 20, 50], duration: float = 30.0
) -> List[LoadTestResult]:
    ...
    async with LoadTester() as tester:
        for users in users_range:
            ...
            logger.info(
                f"Benchmark completed for {users} users: {result.requests_per_second:.2f} req/s"
            )
