...
class WebSocket:
    ...
    def __init__(self, scope: Dict[str, Any], receive: callable, send: callable) -> None:
        self._scope = scope
    ...
    def url(self) -> str:
        ...
        if (scheme == "ws" and server[1] != 80) or (scheme == "wss" and server[1] != 443):
            url += f":{server[1]}"
        url += path
        if query_string:
            url += f"?{query_string}"
    ...
    async def receive_text(self) -> str:
        ...
        if message["type"] == "websocket.receive":
            ...
            raise WebSocketException(1002, f"Unexpected message type: {message['type']}")
