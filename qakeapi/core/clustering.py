...
class RedisClusterNode(ClusterNode):
    ...
    async def broadcast(self, message: ClusterMessage) -> None:
        ...
        await self.redis_client.publish(
            self.cluster_channel, json.dumps(message.to_dict())
        )
            ...
            async for message in listener:
                if message["type"] == "message":
                    try:
                        ...
                        if (
                            cluster_message.target_nodes
                            and self.node_id not in cluster_message.target_nodes
                        ):
                            continue
    ...
    async def _announce_presence(self) -> None:
        ...
        join_message = ClusterMessage(
            ...
            data={"node_id": self.node_id, "timestamp": datetime.now().isoformat()},
            source_node=self.node_id,
        )
...
def create_clustered_manager(
    node_id: str, redis_url: str = "redis://localhost:6379", local_manager: Any = None
) -> ClusteredWebSocketManager:
