            ...
            for route_path, method_map in self._routes_dict.items():
                ...
                if params is not None:
                    if method in method_map:
                        logger.debug(
                            f"Found matching route: {route_path} with method: {method}"
                        )
                        ...
                        logger.debug(
                            f"Method {method} not allowed for {route_path}. Allowed: {list(method_map.keys())}"
                        )
