...
class CORSMiddleware:
    ...
    def _get_origin_from_headers(self, headers: List[tuple[bytes, bytes]]) -> Optional[str]:
    ...
    def _build_preflight_response(self, request: Request) -> Response:
        ...
        headers = [
            ...
            (b"Access-Control-Allow-Methods", ", ".join(self.config.allow_methods).encode()),
            (b"Access-Control-Max-Age", str(self.config.max_age).encode()),
            (b"Access-Control-Allow-Headers", ", ".join(self.config.allow_headers).encode()),
        ]
        ...
        if self.config.expose_headers:
            headers.append(
                (b"Access-Control-Expose-Headers", ", ".join(self.config.expose_headers).encode())
            )
    ...
    async def __call__(self, request: Request, handler) -> Response:
        """Process the request"""
        origin = self._get_origin_from_headers(request.scope["headers"])
        logger.debug(f"Processing request: {request.method} {request.path} origin: {origin}")
