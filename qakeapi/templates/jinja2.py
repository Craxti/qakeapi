        ...
        **kwargs,
        ...
        if self.live_reload_enabled:
            try:
                ...
                start_live_reload()
                logger.info(
                    f"Live reload enabled for template directory: {template_dir}"
                )
            except ImportError:
                logger.warning(
                    "Live reload requires watchdog. Install with: pip install watchdog"
                )
                self.live_reload_enabled = False

        logger.debug(
            f"Initialized Jinja2 template engine with template_dir: {template_dir}"
        )
