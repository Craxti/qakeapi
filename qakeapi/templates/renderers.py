...
class CachedTemplateEngine(Jinja2TemplateEngine):
    ...
    def __init__(
        self, *args, enable_cache: bool = True, cache_size: int = 100, **kwargs
    ):
...
def template_debug(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            end_time = time.time()
            logger.debug(
                f"Template rendering took {(end_time - start_time) * 1000:.2f}ms"
            )
            return result
...
class TemplateDebugger:
    ...
    def debug_render(self, template_name: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Template '{template_name}' rendered in {(end_time - start_time) * 1000:.2f}ms"
            )
    ...
    def debug_render_string(self, template_string: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Template string rendered in {(end_time - start_time) * 1000:.2f}ms"
            )
