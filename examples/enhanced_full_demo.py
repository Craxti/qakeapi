...
from qakeapi import (
    ...
    CORSMiddleware, LoggingMiddleware, CompressionMiddleware,
    MetricsCollector, MetricsMiddleware, create_metrics_endpoint,
    HealthChecker, HealthCheckMiddleware, DiskSpaceHealthCheck, MemoryHealthCheck,
    RateLimitMiddleware, RateLimiter,
    CacheManager, CacheMiddleware,
    ErrorHandler, create_error_handler,
    DataValidator, StringValidator, IntegerValidator,
    StaticFiles, TemplateRenderer,
    status
)
...
items_db = [
    {"id": 1, "title": "Ноутбук", "description": "Мощный ноутбук", "price": 50000, "category": "electronics"},
    {"id": 2, "title": "Книга", "description": "Интересная книга", "price": 500, "category": "books"},
]
...
async def home():
    """Главная страница"""
    return templates.render("enhanced_demo.html", {
        "title": "QakeAPI Enhanced Demo",
        "features": [
...
async def get_user(user_id: str):
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Неверный формат ID"},
            status_code=status.BAD_REQUEST
        )
...
async def search(q: str, type: str = "all"):
    ...
    if type in ["all", "items"]:
        results["items"] = [
            ...
            if q.lower() in item["title"].lower() or 
...
async def get_stats():
    ...
    return {
        "users": {
            "total": len(users_db),
            "latest": users_db[-1] if users_db else None
        },
        "items": {
            "total": len(items_db),
            "categories": list(set(item.get("category", "unknown") for item in items_db))
        },
        "system": {
