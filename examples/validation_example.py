...
product_validator = DataValidator(
    {
        ...
        "tags": ListValidator(
            item_validator=StringValidator(max_length=30), max_items=10, required=False
        ),
...
async def create_user(request: Request):
    ...
    new_user = {"id": len(users_db) + 1, **result.data}
...
async def create_product(request: Request):
    ...
    category_exists = any(
        cat["id"] == result.data["category_id"] for cat in categories_db
    )
    if not category_exists:
        raise ValidationException(
            f"Category with ID {result.data['category_id']} does not exist"
        )

    # Создаем продукт
    new_product = {"id": len(products_db) + 1, **result.data}
...
async def create_user_with_decorator(request: Request, validated_data: dict):
    ...
    # Создаем пользователя
    new_user = {"id": len(users_db) + 1, **validated_data}
    users_db.append(new_user)

    return {"message": "User created successfully with decorator", "user": new_user}
...
if __name__ == "__main__":
    ...
    print(
        '   Пользователь: POST /users/ с {"name": "Иван", "email": "ivan@example.com", "age": 25}'
    )
    print(
        '   Продукт: POST /products/ с {"title": "Ноутбук", "price": 50000, "category_id": 1}'
    )
