...
async def create_mock_request(method, path, query_string="", body=None, headers=None):
    ...
    if body:
        receive.side_effect = [
            {"type": "http.request", "body": body.encode() if isinstance(body, str) else body, "more_body": False}
        ]
    else:
        receive.side_effect = [
...
async def test_endpoint(method, path, query_string="", body=None, headers=None):
    ...
    try:
        scope, receive, send = await create_mock_request(method, path, query_string, body, headers)
